    ...
    this.atFault = function () {
        ...
        for (var i = 1;i <= 9;i++) {
            ...
            if (isSend == false){
                ...
                basicState = new Array();
                basicState[STATUS_r] = '该帧序号为 {i} == i <br>'.replace("{i}", i);
                basicState[LINE] = [14];
                statelist.push(basicState);

                basicState = new Array();
                basicState[STATUS_r] = '接收该帧，发送序号为 {i} 的确认帧&nbsp;i++ <br>'.replace("{i}", i);
                basicState[LINE] = [15,16,17];
                statelist.push(basicState);
                ...
                if ( isReceive ==false){
                    ...
                    basicState = new Array();
                    basicState[STATUS] = '规定时间内,收到序号为 {i} 的确认帧，i++<br>'.replace("{i}", i);
                    basicState[STATUS_r] = '等待帧到达<br>';
                    basicState[LINE] = [12,5,6];
                    statelist.push(basicState);
            ...
            basicState = new Array();
            basicState[STATUS] = '清空计时器<br>';
            basicState[LINE] = [7,12];
            statelist.push(basicState);
    ...
    this.SlidingWindow = function () {
        ...
        for (var i = 1,j = 1;i <= 9;i++) {
            ...
            if (isSend == false){
                ...
                if( i == j ){
                    basicState = new Array();
                    basicState[STATUS_r] = '该帧序号为 {i} == i 为期望接收的帧<br>'.replace("{i}", i);
                    basicState[LINE] = [13];
                    statelist.push(basicState);

                    basicState = new Array();
                    basicState[STATUS_r] = '接收该帧,发送序号为 {i} 的确认帧,滑动接收窗口<br>'.replace("{i}", i);
                    basicState[LINE] = [14,15,16,17];
                    statelist.push(basicState);
                    j++;
                }else {
                    basicState = new Array();
                    basicState[STATUS_r] = '该帧序号为 {i} != {j} 不是期望接收的帧<br>'.replace("{i}", i).replace("{j}", j);
                    basicState[LINE] = [13];
                    statelist.push(basicState);

                    basicState = new Array();
                    basicState[STATUS_r] = '不滑动接收窗口，发送序号为{i}的确认帧'.replace("{i}", i);
                    basicState[LINE] = [17];
                    statelist.push(basicState);
                }
                ...
                if ( isReceive[i-1] == 1){
                    ...
                    basicState = new Array();
                    basicState[STATUS] = '收到确认帧{i},滑动发送窗口,停止窗口外帧的计时器<br>'.replace("{i}", i);
                    basicState[STATUS_r] = '等待帧到达<br>';
